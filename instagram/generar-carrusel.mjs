...
// → cierre con llamada a la web. No es la misma tarjeta repetida diez
// veces: cada lámina hace un trabajo distinto en la historia.
...
/* --------------------------------------------------------------------
   ...
   Cabecera: fuente/marca a la izquierda, contador a la derecha — el
   contador es lo que le dice a quien mira que hay más láminas y que
   merece la pena deslizar.
   -------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------
   ...
   Titular corto: el contexto ya lo puso la lámina 1, así que aquí no
   hace falta repetir "mandar 200 dólares a..." completo.
   -------------------------------------------------------------------- */

function laminaExtremo({ indice, total, etiqueta, pais, datoValor, costoPct }) {
  const cuerpo = `
    <p class="kicker">${etiqueta}</p>
    <p class="titular-extremo">${pais}</p>
...
/* --------------------------------------------------------------------
   ...
   Sin cifra: el dato central de esta lámina es la ausencia, así que los
   tres nombres van en rojo en vez de un número.

   Solo dice lo que limpio.json puede sostener: que la serie no tiene
   valor para estos tres países en 2023. No distingue "sin indicador" de
   "valor nulo" porque esa distinción no está en el archivo que lee este
   script — vive en metodo.md, no aquí. Si se quiere esa matización en la
   imagen, hay que traerla de metodo.md a mano.
   -------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------
   LÁMINA 5 — Cierre
   La única con cita completa. La URL apunta a la portada, no a una
   página de la pieza: esa página todavía no existe en el sitio. Cuando
   se migre "remesas" a Astro (como ya se hizo con "el título no cruza"),
   cambiar el enlace a la ruta de la pieza.
   -------------------------------------------------------------------- */
